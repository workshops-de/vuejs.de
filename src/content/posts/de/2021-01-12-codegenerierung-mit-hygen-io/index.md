...
published_at: 2021-01-12T20:00:00.000Z
...
Oder direkt ausgeführt über den Package Runner [npx](https://medium.com/@maybekatz/introducing-npx-an-npm-package-runner-55f7d4bd282b):
...
Innerhalb dieser befinden sich die sogenannten Metadaten: der _head_ der Datei. Dieser entspricht der YAML-Syntax.
Darunter befindet sich der zu generierende Inhalt der Zieldatei: der _body_. Die hier verwendete Sprache kann frei gewählt werden. Wir verwenden JavaScript.

Wenn wir uns jetzt an die Datei-Ausgabe unter `app/hello.js` zurückerinnern, sollten wir viele Zusammenhänge erkennen. Zum einen entspricht der Inhalt dieser Datei genau dem _body_ unseres Generators und zum anderen wird durch den im _head_ Bereich angegebenen Befehl der Ablageort definiert:
...
Diese beiden Anpassungen können wir relativ einfach vornehmen, indem wir den _head_ Bereich der Datei `hello.ejs.t` wie folgt verändern:
...
Den Inhalt der generierten Datei können wir ebenso einfach austauschen. Dafür kopieren wir den weiter oben gezeigten Code unserer Beispielkomponente und fügen diesen in den _body_ Bereich der Datei `hello.ejs.t` ein:
...
Die gesamte Template-Datei wird von der Template-Engine [EJS](https://ejs.co) verarbeitet, sodass uns hier noch mehr Möglichkeiten zur Verfügung stehen.
...
Gebt uns gerne [Feedback](https://vuejs.de/discord), ob Ihr noch mehr in diese Richtung erfahren wollt. Mit hygen lassen sich noch viel umfangreichere Funktionen umsetzen, wie zum Beispiel:

1. Nutzung von _prompts_: Damit können vom Nutzer zusätzliche Eingaben abgefragt werden, um die Code-Generierung flexibler zu gestalten und dynamische Inhalte abbilden zu können.
2. Die Erzeugung mehrerer Dateien innerhalb eines Generators.
3. Automatisches Hinzufügen von Node Dependencies beim Ausführen von Generatoren, sogenannte _injects_.
4. und vieles mehr
